...
function getAdTheme(theme?: string | null) {
  if (theme === "red") return "bg-[rgb(var(--accent))] text-white";
  if (theme === "dark") return "bg-black text-white";
  if (theme === "gold") return "bg-[#E7B21D] text-black";
    if (theme === "tv19")
...
function getMutedText(theme?: string | null) {
  return theme === "gold" ? "text-black/55" : "text-white/65";
}

function getBodyText(theme?: string | null) {
  return theme === "gold" ? "text-black/80" : "text-white/90";
}
...
export default function AdBox({
  ...
  const content = (
    <aside
      ...
      className={[
        "group relative overflow-hidden p-5 shadow-sm",
        "min-h-[260px]",
    ...
    >
      ...
      <div className="relative z-10 flex min-h-[220px] flex-col">
        <div className={`text-[10px] font-black uppercase tracking-[0.28em] ${getMutedText(ad.theme)}`}>
          {ad.label || "ANNONSE"}
        </div>

        {ad.sponsor ? (
  <div className="mt-2 text-xs font-black uppercase tracking-[0.18em] text-white/55">
    {ad.sponsor}
  </div>
) : null}
...
{ad.image_url ? (
  ...
  />
        ...
        <div className="mt-auto">
          ...
          <p className={`mt-3 text-sm font-black leading-snug ${getBodyText(ad.theme)}`}>
            {ad.text}
          </p>
          ...
          {ad.cta ? (
            <div
              className={[
                "mt-5 inline-block border-2 px-3 py-2 text-xs font-black uppercase tracking-[0.12em]",
            ...
            {safeAds.map((item, dotIndex) => (
              <span
                ...
                className={[
                  ...
                  dotIndex === index ? "bg-current opacity-70" : "bg-current opacity-20",
                ].join(" ")}
              />
            ))}
          </div>
